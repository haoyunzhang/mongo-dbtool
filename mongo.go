...
type DataBase struct {
	DBName       string                       //数据库名称
	DBInstance   *mgo.Database                //该数据库对应的数据库实例
...
func (this *Handler) Connect(address string, userName string, passWord string) (err error) {
	...
	this.DBsession, err = mgo.DialWithInfo(mongoDialInfo)
	if err != nil {
...
func (this *Handler) DataBase(DBName string) (result *DataBase) {
	...
	if ok {
		...
		index := mgo.Index{
			Key:        []string{"id", "timestamp"},
			Background: true,
		}
		result.Collection("aircraft").EnsureIndex(index)
	}
